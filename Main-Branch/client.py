...
poweredby = Text(f'''CaveGame [By TT0tem4ik22]\npowered by Ursina''', font=nunito, scale=3, origin=(0,0))
...
info = Text("", parent=camera.ui, scale=1, font=nunito, position=(-1.15,.45))
...
def onConnectionError(Reason):
    ...
    if Reason in reasons:
        NoConnection = Text(f"Connection to {argv[1]}:{argv[2]} error!\nReason: {reasons[Reason]}", parent=camera.ui, font=nunito, color=color.white, scale=3, origin=(0, -2))
    else:
        NoConnection = Text(f"Connection to {argv[1]}:{argv[2]} error!\nReason: {Reason}", parent=camera.ui, font=nunito, color=color.white, scale=3, origin=(0, -2))
    exit_button = Button(
        text="Exit",
